...
var distHeader = '/**\n\
 * newforms-gridforms <%= pkg.version %><%= dev %> - https://github.com/insin/newforms-gridforms\n\
 * MIT Licensed\n\
 */\n'
...
gulp.task('bundle-js', ['clean-dist', 'lint-js'], function() {
  ...
  ,*/ standalone: 'NewformsGridForms'
  , detectGlobals: false
  })
  b.transform('browserify-shim')

  var stream = b.bundle()
    .pipe(source('newforms-gridforms.js'))
    .pipe(streamify(header(distHeader, {pkg: pkg, dev: dev})))
    .pipe(gulp.dest('./dist'))

  if (gutil.env.production) {
    stream = stream
      .pipe(rename('newforms-gridforms.min.js'))
